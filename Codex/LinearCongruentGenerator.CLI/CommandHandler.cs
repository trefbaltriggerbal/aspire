...
public class CommandHandler
{
    ...
    public long Seed => _rng.Seed;

    public void Jump(long steps) => _rng.Jump(steps);

    public string? Set(string param, long value)
    {
            ...
            case "m":
                {
                    ...
                    if (!BuildRng(out var err))
                    {
                        return $"{err}. Use 'set m {old}' to restore the previous value.";
                    }
                }
                break;
            case "a":
                {
                    ...
                    if (!BuildRng(out var err))
                    {
                        return $"{err}. Use 'set a {old}' to restore the previous value.";
                    }
                }
                break;
            case "c":
                {
                    ...
                    if (!BuildRng(out var err))
                    {
                        return $"{err}. Use 'set c {old}' to restore the previous value.";
                    }
                }
                break;
            default:
                return "Unknown parameter. Use m, a or c.";
        }
    ...
    public string? Get(string param)
    {
        ...
        {
            "m" => _multiplier.ToString(),
            "a" => _addition.ToString(),
            "c" => _modulus.ToString(),
            "seed" => _rng.Seed.ToString(),
            _ => null
        };
    }
